      ...
      <div v-else-if="filteredBookings.length" class="flex flex-col gap-4">
        <div v-for="booking in filteredBookings" :key="booking.id" class="card">
          <div class="flex justify-between items-start mb-2 border-b pb-2">
            <div>
              <span class="badge" :class="booking.status === 'confirmed' ? 'badge-success' : (booking.status === 'pending' ? 'badge-warning' : 'badge-secondary')">
                {{ booking.status }}
              </span>
              <p class="text-xs text-light mt-1">Reserva #{{ booking.id }}</p>
            </div>
            <div class="text-right">
              <p class="font-bold text-primary">${{ booking.total_price.toLocaleString('es-MX') }}</p>
              <p class="text-xs text-secondary">{{ booking.guest_count }} persona{{ booking.guest_count > 1 ? 's' : '' }}</p>
            </div>
          </div>
          ...
          <div class="py-2">
            ...
            <div class="flex items-center gap-2 mt-1 text-sm text-secondary">
              <Calendar :size="14" />
              <span>{{ formatDate(booking.schedule_start) }} a las {{ formatTime(booking.schedule_start) }}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 mt-3 pt-3 border-t">
            ...
            <div class="flex-1 flex items-center gap-2">
              <div class="w-8 h-8 rounded-full bg-secondary-50 text-secondary flex items-center justify-center font-semibold text-xs">
                V
              </div>
            ...
            <div class="flex gap-2" v-if="booking.status === 'pending'">
              <button class="btn btn-error btn-xs flex items-center gap-1" @click="updateStatus(booking.id, 'cancelled')">
                <X :size="14" /> Rechazar
              </button>
              <button class="btn btn-success btn-xs flex items-center gap-1" @click="updateStatus(booking.id, 'confirmed')">
                <Check :size="14" /> Confirmar
              </button>
            </div>
            <div v-else-if="booking.status === 'confirmed'">
              <button class="btn btn-primary btn-xs flex items-center gap-1" @click="updateStatus(booking.id, 'completed')">
